...
public class UrlFunctionsBugTest {
    ...
    public void refinementAdditionWithMapping() throws Exception {
        ...
        String url = UrlFunctions.toUrlAdd(DEFAULT_BEAUTIFIER, "", navigations, "category_leaf_expanded",
                                           new RefinementValue().setValue(refinementString).setCount(5483));
        assertEquals(
    ...
    public void testRefinementAdditionWithMapping() throws Exception {
        List<Navigation> navigations = singletonList(new Navigation().setName("category_leaf_expanded").setRefinements(
                singletonList(new RefinementValue().setCount(1).setValue("Category Root~Chapters" +
