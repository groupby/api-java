...
public class UrlBeautifierTest {
    ...
    public void testFromUrlEmptySuffix() throws Exception {
        ...
        assertEquals("this is a test", ((RefinementValue) query.getRefinements().get(0)).getValue());
    }
    ...
    public void testSearchQuery() throws Exception {
        ...
        assertEquals("this is a test", query.getSearchString());
    ...
    public void testSearchUrlBackAndForth() throws Exception {
        ...
        assertEquals("this is a test", query.getSearchString());
        assertEquals("test", query.getRefinements().get(0).getNavigationName());
        assertEquals("value", ((RefinementValue) query.getRefinements().get(0)).getValue());
    }
    ...
    public void testUnappend() throws Exception {
        ...
        assertEquals(2, query.getRefinements().size());
        assertEquals("test=value", query.getRefinements().get(0).toGsaString());
        assertEquals("height=20in", query.getRefinements().get(1).toGsaString());
    }
    ...
    public void testUnmappedFromUrl() throws Exception {
        ...
        assertEquals(5, refinements.size());
        assertEquals("test=value", refinements.get(0).toGsaString());
        assertEquals("height=20in", refinements.get(1).toGsaString());
        assertEquals("category=computer accessories", refinements.get(2).toGsaString());
        assertEquals("category2=mice", refinements.get(3).toGsaString());
        assertEquals("cat3=wireless mice", refinements.get(4).toGsaString());
    }
    ...
    public void testToUrlWithReplaceWithRefinement() throws Exception {
        ...
        test.addReplacementRule('/', ' ');
        test.addReplacementRule('&', ' ', test.SEARCH_NAVIGATION_NAME);
    ...
    private void toAndFromUrl(String pSearchString, String pRefinementString, String... pExpectedRefinementsValues)
        ...
        Query query = test.fromUrl(url);
        assertEquals(pSearchString, query.getSearchString());
        for (int i = 0; i < pExpectedRefinementsValues.length; i++) {
            String refinement = pExpectedRefinementsValues[i];
            assertEquals(refinement, ((RefinementValue) query.getRefinements().get(i)).getValue());
        }
    }
    ...
    private void testFromUrl(String purl, String searchString, String... refinements) throws Exception {
        ...
        if (StringUtils.isNotEmpty(searchString)) {
            assertEquals(searchString, query.getSearchString());
        }
        for (int i = 0; i < refinements.length; i++) {
            assertEquals(refinements[i], ((RefinementValue) query.getRefinements().get(i)).getValue());
        }
    }
}
